...
"""
This function creates a metadata file that contains:
    * num_utterances 
    * max_utterance_len 
    * vocabulary_size 
    * num_da_tags 
    * max_number_utterances 
    * index_to_word 
    * word_to_index 
    * index_to_label
    * label_to_index 
...
# splits by Lee & Dernocourt (2016) (arXiv preprint arXiv:1603.03827 .)
if data == 'SwDA':
    ...
    save_directory = "data\\processed_data\\SwDA\\"

    def combine_data(data_dir, batch):
        """
        This function combines the separate .txt files into a 
        pandas dataframe for each batch. 

        Stores the maximum number of utterances in a dialogue
        """
        if batch == 'train':
            set_idx = train_set_idx
        elif batch == 'test':
            set_idx = test_set_idx
        elif batch == 'valid':
            set_idx = valid_set_idx

        max_number_utterances = 0
        data = pd.read_csv(data_dir + set_idx[0].split('w')[1] + '.txt', sep = '|', header = None, names = ['speaker', 'utterance', 'DA'])
        for i in range(1,len(set_idx)):
            new_data = pd.read_csv(data_dir + set_idx[i].split('w')[1] + '.txt', sep = '|', header = None, names = ['speaker', 'utterance', 'DA'])
            if len(new_data) > max_number_utterances:
                max_number_utterances = len(new_data)
            
            data = data.append(new_data, ignore_index = True)

        return data, max_number_utterances
    ...
    save_directory = "data\\processed_data\\MRDA\\"
    ...
    max_number_utterances = 0
    data = pd.read_csv(data_dir + set_idx[0] + '.txt', sep = '|', header = None, names = ['speaker', 'utterance', 'DA', 'general DA', 'full DA'])
    for i in range(1,len(set_idx)):
        new_data = pd.read_csv(data_dir + set_idx[i] + '.txt', sep = '|', header = None, names = ['speaker', 'utterance', 'DA','general DA', 'full DA'])
        if len(new_data) > max_number_utterances:
            max_number_utterances = len(new_data)
...
train = combine_data(data_dir, batch = 'train')
test = combine_data(data_dir, batch = 'test')
valid = combine_data(data_dir, batch = 'valid')

train_data = train[0]
test_data = test[0]
valid_data = valid[0]

# get maximum number of utterances
max_number_utterances = max(train[1], test[1], valid[1])

# create full dataset
swda_data = train_data.append([test_data, valid_data])

# create lists with all utterances and all da_tags
utterances = []
da_tags = []
max_utterance_len = 0

swda_data['utterance'] = swda_data['utterance'].str.lower()

for i in range(0,len(swda_data)):
    utterance = swda_data.iloc[i,1]
    da = swda_data.iloc[i,2]
...
num_labels = len(labels) # Count number of labels
...
metadata = dict(
    ...
    num_da_tags = num_labels,
    max_number_utterances = max_number_utterances,
...
pickle.dump(metadata, open(save_directory + 'metadata.pkl', 'wb'))
