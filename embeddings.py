"""
This function creates word2vec embedding matrices. We use three type of embeddings:

(1) trained embeddings
	a) 200D
	b) 300D
(2) pre-trained Google News 300D
...
if data == 'SwDA':
    ...
    unprocessed_data_dir = "data\\raw_data\\SwDA\\"
    processed_data_dir = "data\\processed_data\\SwDA\\"
    embedding_data_dir = "data\\embeddings\\SwDA\\"
elif data == 'MRDA':
    ...
    # set data directories
    unprocessed_data_dir = "data\\raw_data\\MRDA\\"
    processed_data_dir = "data\\processed_data\\MRDA\\"
    embedding_data_dir = "data\\embeddings\\MRDA\\"

metadata = pickle.load(open(processed_data_dir + "metadata.pkl", "rb"))

# pre-trained model directory
pre_trained_dir = 'data\\embeddings\\pre-trained\\'


#------------------ (1) Trained embeddings ---------------------#
def get_utterances(data_dir, set_idx):
    data_list_x = []
    for i in range(0,len(set_idx)): 
        ...
        tokenized_utterances = [nltk.word_tokenize(x) for x in new_data['utterance'].str.lower() if np.str(x) != 'nan']
        
        data_list_x = data_list_x + tokenized_utterances
    return data_list_x

# get all utterances
utterances = get_utterances(unprocessed_data_dir, train_set_idx + valid_set_idx + test_set_idx )

#-----200D-----------#
dimensions = 200

# train model with 200D
model_200D = Word2Vec(utterances, size = dimensions, window = 5,  sg = 1, hs = 0, negative = 10, alpha=0.025, iter = 5, min_count=1)

# get index_to_word
index_to_word = metadata['index_to_word']

# create embedding matrix
embedding_matrix = np.zeros((len(model_200D.wv.vocab)+1, dimensions))
for i in range(1,len(model_200D.wv.vocab)+1):
    embedding_vector = model_200D.wv[index_to_word[i]] # USE OWN INDEX-TO-WORD!!!!
    if embedding_vector is not None:
        embedding_matrix[i] = embedding_vector

print('embedding matrix build')
print(embedding_matrix.shape)

# save embedding matrix
embd_matrix_train_200D = pickle.dump(embedding_matrix, open(embedding_data_dir + 'embd_matrix_train_200D.pkl', 'wb'))
...
# save embedding matrix
embd_matrix_train_300D = pickle.dump(embedding_matrix, open(embedding_data_dir + 'embd_matrix_train_300D.pkl', 'wb'))

#------------------ (1) Pre-trained embeddings ---------------------#
# load pre-trained model
embedding_model = KeyedVectors.load_word2vec_format(pre_trained_dir + 'GoogleNews-vectors-negative300.bin.gz', binary=True)

# construct empty embedding matrix
word_to_index = metadata['word_to_index']
vocabulary_size = metadata['vocabulary_size']
dimensions = 300
embedding_matrix = np.zeros((vocabulary_size+1, dimensions))

# for each vocabulary position i in embedding matrix
# insert the 1x300 pre-trained word2vec GoogleNews embedding
...
# save embedding matrix
embd_matrix_pre_train_Google_300D = pickle.dump(embedding_matrix, open(embedding_data_dir + 'embd_matrix_pre_train_Google_300D.pkl', 'wb'))
